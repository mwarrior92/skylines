...
def sites_covered():
    ...
    x, y, z = list(zip(*xy))
    with open('sites_covered.json', 'w+') as f:
        json.dump({'doms_used': x, 'sites_covered': y, 'median_links_covered': z}, f)

    '''
    ax.plot(x,y)
    ax.set_xlabel('# of domains used')
    ax.set_ylabel('# of sites covered')
    ax.set_xlim([0, 200])
    ax.set_ylim([0, 1500])
    ax2 = ax.twinx()
    ...
    ax2.set_ylabel('median % of links from covered sites')
    fig.savefig('doms_vs_sites_covered.png')
    plt.close(fig)
    '''
...
def distinct_links():
    ...
    '''
    ax.plot(x,y)
    ax.set_xlabel('# link domains on site')
    ax.set_ylabel('CDF of sites')
    fig.savefig('link_doms_per_site_cdf.png')
    plt.close(fig)
    '''
